...
class ParseHooksTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testEmptyFetchFunctions()
    {
        $this->assertEmpty(self::$hooks->fetchFunctions());
    }

    public function testSingleCloudFunctionNotFound()
    {
        $this->setExpectedException('Parse\ParseException', 'no function named: sendMessage is defined', 143);
        self::$hooks->fetchSingleFunction('sendMessage');
    }
    ...
    public function testSingleCloudTriggerNotFound()
    {
        $this->setExpectedException('Parse\ParseException', 'class Scores does not exist', 143);
        self::$hooks->fetchSingleTrigger('Scores', 'beforeSave');
    }
